...
par(mar=c(4,1,2.5,1))
plot(10,10,xlim=c(-9,log((1-ci-cm)/co)+1),ylim=log(c(co*(cm+2*ci)/(ci+cm),co/cm)),xlab=expression(tau),ylab="",xaxt="n",yaxt="n",cex.lab=1.5)
...
curve(log(Vectorize(d_l_varphi_A)(exp(x))),log(ci*cm/(co*(1-2*ci))*0.1),log((1-ci-cm)/co),add = T,col="purple",n=100,lty=2,lwd=3)
curve(log(Vectorize(d_l_varphi_V)(exp(x))),log(ci*cm/(co*(1-2*ci))*0.1),log((1-ci-cm)/co),add = T,col="blue",n=100,lty=4,lwd=3)
legend("bottomleft",legend=expression(K(tau),varphi[D],varphi[A],varphi[V]),lty=c(1,3,2,4),col=c("black","red","purple","blue"),lwd=2)
dev.off()
